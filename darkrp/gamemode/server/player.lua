...
function meta:ChangeTeam(t, force)
	...
	if (t == TEAM_armsdealer and not self:CheckGroup("respected") and not self:CheckGroup("adminrespected")) then
	GAMEMODE:Notify(self, 1, 4, "You must donate to become this class!")
		return
	end
		if (t == TEAM_SWAT		and not self:CheckGroup("respected") and not self:CheckGroup("adminrespected")) then
	GAMEMODE:Notify(self, 1, 4, "You must donate to become this class!")
		return
	end
		if (t == TEAM_doubleagent and not self:CheckGroup("respected") and not self:CheckGroup("adminrespected")) then
	GAMEMODE:Notify(self, 1, 4, "You must donate to become this class!")
		return
	end
		if (t == TEAM_hitman and not self:CheckGroup("respected") and not self:CheckGroup("adminrespected")) then
	GAMEMODE:Notify(self, 1, 4, "You must donate to become this class!")
		return
	end
